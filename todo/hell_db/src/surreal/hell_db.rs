...
use hell_error::{HellResult, HellError, ErrToHellErr};
...
impl HellDb {
    pub async fn new(info: HellDbInfo) -> HellResult<Self> {
        let datastore = Datastore::new(&info.path).await.to_hell_err(hell_error::HellErrorKind::DbError)?;
...
impl HellDb {
    ...
    async fn execute_internal(&self, txt: &str, vars: Option<BTreeMap<String, sql::Value>>) -> HellResult<HellDbResponse> {
        HellDbResponse::new(
            self.datastore.execute(txt, &self.session, vars, self.strict).await.to_hell_err(hell_error::HellErrorKind::DbError)?
        )
    }
    ...
    async fn process_internal(&self, ast: Query, vars: Option<BTreeMap<String, sql::Value>>) -> HellResult<HellDbResponse> {
        HellDbResponse::new(
            self.datastore.process(ast, &self.session, vars, self.strict).await.to_hell_err(hell_error::HellErrorKind::DbError)?
        )
    }

    /// Ensure a SQL Value is fully computed
    async fn compute_internal(&self, val: sql::Value, vars: Option<BTreeMap<String, sql::Value>>) -> HellResult<sql::Value> {
        Ok(self.datastore.compute(val, &self.session, vars, self.strict).await.to_hell_err(hell_error::HellErrorKind::DbError)?)
    }

    /// Performs a full database export as SQL
    async fn export_internal(&self, ns: String, db: String, chn: surrealdb::channel::Sender<Vec<u8>>) -> HellResult<()> {
        Ok(self.datastore.export(ns, db, chn).await.to_hell_err(hell_error::HellErrorKind::DbError)?)
    }
}
